...
/*
void softShadows(RayTriangleIntersection intersection, float gradientConstant){
  ModelTriangle triangle = intersection.intersectedTriangle;
  vec3 normal = getNormalOfTriangle(triangle);
  ...
  // we are in-between, and need to calculate the gradient
    ...
    // calculate the intersection point between the normal vectors and the light ray
    // p1 + (t1*d1) = p2 + (t2*d2)
    vec3 x1 = point;
    vec3 x2 = point + up;
    vec3 x3 = 
